...
class ComingTvShowsViewHolder(view:View, private val listener: OnShowMovieSelectedListener?):RecyclerView.ViewHolder(view) {
    ...
    fun bind(data: ResultTV?, genres:List<Genre>?){
        ...
        ViewCompat.setTransitionName(binder.comingPoster, "headPoster${data?.id}")
        ViewCompat.setTransitionName(binder.comingTitle, "headTitle${data?.id}")
    }
    ...
    companion object{
        fun create(parent:ViewGroup, listener: OnShowMovieSelectedListener?):ComingTvShowsViewHolder{
            val inflater = LayoutInflater.from(parent.context)
            val binding = ComingItemBinding.inflate(inflater, parent, false)
            return ComingTvShowsViewHolder(binding.root, listener)
        }
    }
}